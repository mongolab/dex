...
Dex is a MongoDB performance tuning tool that compares logged queries to the
available indexes in the queried collection(s) and generates index suggestions
based on simple heuristics. To use Dex, you provide--at a minimum--a path to
your MongoDB log file and a connection URI for your database.
...
> dex -f my/mongod/data/path/mongodb.log -n "*.collectionOne" mongodb://myUser:myPass@myHost:12345/admin
...
found in the logfile to the indexes available in the database, recommending
indexes for those queries which are not indexed. Recommended for MongoDB version
2.0.4 or later.
...
For logfile input, provide logfiles generated by mongod 2.0.4 or later.
...
In the future, Dex will support using the system.profile collection instead of (or in addition to) a log file. Example:

```
> dex -p -n "myFirstDb.*" -n "mySecondDb.*" mongodb://myUser:myPass@myHost:12345/admin
```
